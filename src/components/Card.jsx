...
import DeleteCard from "../Redux/todos/Thunk/DeleteCard";

 

const Card = ({todoitem, EditHandler}) => {
    const dispatch = useDispatch()
    const {id,name, author, thumbnail, price, rating=0, featured} = todoitem 


    const deleteHandler = (todoId) => {

      dispatch(DeleteCard(todoId))
          ...
          <div className="flex items-center justify-between">
           {featured && <span className="badge-success lws-Badge">featured</span>} 
            <div className="text-gray-500 space-x-2">
              <button className="lws-edit" onClick={()=>EditHandler(todoitem)}>
              ...
              <button className="lws-delete" onClick={() => deleteHandler(id)}>
                <svg fill="none" viewBox="0 0 24 24" stoke-width="1.5" stroke="currentColor" className="w-6 h-6">
                  <path stroke-linecap="round" stroke-linejoin="round"
